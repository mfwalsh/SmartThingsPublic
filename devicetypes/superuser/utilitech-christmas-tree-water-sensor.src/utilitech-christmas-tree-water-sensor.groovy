 ...
 *
 *  Author: tosa68, GUI modifications by Steve White
 *  Date:   2016-12-9 (revised SW)
 *
 *  Features:
 *
...
metadata {
	definition (name: "Utilitech Christmas Tree Water Sensor", author: "shackrat") {
	...
	tiles {
		standardTile("water", "device.water", width: 2, height: 2) {
			state "dry", label: "dry", icon:"st.alarm.water.wet", backgroundColor:"#ff0000"
			state "wet", label: "OK", icon:"st.alarm.water.dry", backgroundColor:"#53a7c0"
		}
...
def zwaveEvent(physicalgraph.zwave.commands.batteryv1.BatteryReport cmd) {
	...
	if (cmd.batteryLevel == 0xFF) {          // Special value for low battery alert
		...
		map.descriptionText = "${device.displayName} battery level ${cmd.batteryLevel}%"
        map.isStateChange = true
		map.displayed = true
	}
